...
Add a model: give it `build()`, register in `MODEL_BUILDERS`, then `MODEL_REGRESSION_REBASELINE=1 pytest tests/ -m regression` to create `signature.json`; re-baseline the same way to bless intentional geometry changes. The suite carries a `regression` marker so it is excluded from the frequent `pytest tests/` run (pytest.ini `addopts = -m "not regression"`); it runs via `-m regression` and through `/commit`, which runs `.claude/commit-checks.sh` (base suite then regression). Import-time-export models must first move export into `build()`/`__main__` — importing them otherwise runs a full export to committed paths (e.g. `grand_austria_hotel` still exports at import).
